...
async function up(docker, projectName, recipe, output, options, loadYaml) {
  ...
  for (var serviceName of serviceNames) {
    ...
    if (service.networks !== undefined) {
      servicesTools.buildNetworks(projectName, serviceName, service.networks, networksToAttach);

      const networkToAttachKey = Object.keys(networksToAttach[0])[0];
      const networkToAttachInfo = findNetworkInfo(output, networkToAttachKey);
      
      if (typeof networkToAttachInfo.networkName !== 'undefined') {
        opts.NetworkingConfig.EndpointsConfig = {};
        opts.NetworkingConfig.EndpointsConfig[networkToAttachInfo.networkName] = networksToAttach[0][networkToAttachKey];
      } else {
        opts.NetworkingConfig.EndpointsConfig = networksToAttach[0];
      }
...
var findNetworkInfo = function (output, name) {
...
var findNetwork = function (output, name) {
  const network = findNetworkInfo(output, name);
